        ...
        if (amount >= 0):
            ...
            if rollD4 == 1:
                if amount <= 0 :
                    ...
                    print(" 🔨🛡️ Bonus : Holy Armor reduces wounds by 3")
                    amount -= 3
            elif rollD4 == 2:
                randomAttack = randint(-3,5)
                self._attack_value = self._attack_value + randomAttack
                print(f" 💢🔨✨ Bonus : The Paladin calls to the Holy light to gain attack, {self._attack_value} : {randomAttack}")
            elif rollD4 == 3:
                randomDefense = randint(-1,7)
                self._defense_value = self._defense_value + randomDefense
                print(f" 🛡️🔨✨ Bonus : The Paladin calls to his faith to gain defense, {self._defense_value} : {randomDefense}")
            else:
...
class Phantom_Warden(Character):
    ...
    def compute_wounds(self, damages, roll, attacker):
       ...
       if (roll == self._dice._faces):
           print(" 🌫️ 👻💨 Bonus : The Phantom_Warden evaporated throught the attack ! (Wounds : 0)")
           return super().compute_wounds(0, 0, attacker)   
       elif (roll == 1):
           print("🔻👻 Malus : The Phantom_Warden missed is escape ... (Wounds x 2)")
           return round(super().compute_wounds(damages, roll, attacker) * 2)
       else:
           print(f" 👻💨 Bonus : The Phantom_Warden evaded the attack ! (Wounds - {roll})")
...
class Leviathan(Character):
    ...
    def decrease_health(self, amount):
        if (self._current_health < self._max_health):
           ...
           if coinThrow == 1:
               print("🔻🐙 Malus : The Leviathan failed to restore health points")            
           else : 
               restore_roll = randint(1,10)
               if (restore_roll == 10):
                    print("✨🐙 Bonus : The Leviathan restored 4 health points")
                    if (self._current_health + restore_roll > self._max_health):
                        self._current_health = self._max_health
                    else:
                        self._current_health += 10  
               elif (restore_roll == 1):
                  print(" 🔻🐙 Malus : The Leviathan didn't succeed to restore health points")
                  
               else:
                    print(f"✨🐙 Bonus : The Leviathan restored {restore_roll} health points")
                    if (self._current_health + restore_roll > self._max_health):
                        self._current_health = self._max_health
                    else:
...
class Guardian(Character): # Enforcer 
    ...
    def decrease_health(self, amount):
        if (self._current_health - amount) < 0:
            ...
            rollD4 = randint(1,4)
            if not rollD4 == 4:
                print(" 🔻♖ The Guardian failed to gain defense as he's weakened by the Attacker")
            else:
...
class Shield_Master(Character):
   def compute_wounds(self, damages, roll, attacker: Character):
        rollD6 = randint(1,6)
        if rollD6 == 1:
            print(" 🔻🛡️ Malus : The Shield_Master didn't succeeded in his parry")
            return super().compute_wounds(damages, roll, attacker)
        elif rollD6 == 6:
            attacker.decrease_health(damages)
            print(f" 💢🛡️ Bonus : The Shield_Master parried your attack for a Critical Hit ! (Sends back {damages})") 
            return super().compute_wounds(0, 0, attacker)
        else:
            print(f" 💫🛡️ Bonus : The Shield_Master parried your attack ! ({damages}/{rollD6} ≈ {round(damages/rollD6)})")
